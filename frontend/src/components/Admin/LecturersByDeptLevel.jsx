...
export default function LecturersByDeptLevel() {
  const [data, setData] = useState([]);
  const [loading, setLoading] = useState(false);
  const [message, setMessage] = useState("");
  const fetchLecturersDepartments = async () => {
    setLoading(true);
    const csrf_token = await fetchFastCsrfToken();
    try {
      const res = await axios.get(
        `${API_URL}/admin/lecturers-by-departments-levels`,
        {
          headers: {
  ...
  useEffect(() => {
    fetchLecturersDepartments();
  }, []);

  return (
    <Container className="mt-4">
      {message && <div className="alert alert-info">{message}</div>}
      <h4>Lecturers by Department & Level</h4>
      {loading ? (
        <Spinner />
      ) : (
                    ...
                    >
                      ...
                      <Accordion.Body>
                        <Table striped bordered hover variant="dark" responsive>
                          ...
                          <tbody>
                            {level.lecturers.map((lecturer, idx) => (
                              <tr key={lecturer.id}>
                                <td>{idx + 1}</td>
                                <td>{lecturer.name}</td>
