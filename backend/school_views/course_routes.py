...
from model import Department, Level, User,  LecturerDepartmentAndLevel, Course, StudentLevelProgress
from schema import Role, CourseResponse
...
class CourseRoutes:
    ...
    async def create_course(self,
        ...
        assigned_departments = db.query(LecturerDepartmentAndLevel).filter_by(
            lecturer_id=current_user).all()
        ...
        assigned_department_ids = [
            ad.department_id for ad in assigned_departments]
        ...
        level = db.query(Level).filter_by(
            id=level_id, department_id=department_id).first()
        if not level:
            raise HTTPException(
                status_code=404,
        ...
        db.commit()
        db.refresh(course)
        asyncio.create_task(manager.broadcast_to_department(
            {"event": "course_created", "message": f"New course '{course.title}' created by {self.current_user.name}", "course_id": course.id, "department_id": department_id}, department_id))
    ...
    async def update_course(self, course_id: int, title: str = Form(...), description: str = Form(...), level_id: int = Form(...), syllabus: UploadFile = File(None)):
        self._check_lecturer()
        current_user = self.current_user.id
        db = self.db

        course_update = db.query(Course).filter(
            Course.id == course_id,
            Course.lecturer_id == current_user
        ).first()
    ...
    def browse_courses(self):
        ...
        student_levels = self.db.query(StudentLevelProgress).filter_by(
            student_id=self.current_user.id).all()
        ...
        courses = self.db.query(Course).filter(
            Course.level_id.in_(level_ids)).options(
