...
from model import User, StudentResult, Enrollment, AssignmentGrade, AssignmentSubmission, Course
from schema import Role,  StudentResultOut, StudentResultSubmissionSchema, StudentAddResultOut, StudentOut
...
class StudentResultRoute:
    ...
    async def add_or_update_result(self, results: Union[List[StudentResultSubmissionSchema], StudentResultSubmissionSchema]):
        ...
        for result in results:
            ...
            stmt = (select(StudentResult).options(selectinload(StudentResult.student)).where(
                StudentResult.student_id == result.student_id, StudentResult.course_id == result.course_id))
            ...
            student_stmt = (
                select(User)
                .where(User.id == student_result.student_id, User.role == Role.STUDENT)
            )
            ...
            output_results.append(StudentAddResultOut(
    ...
    @result_router.get("/results/course/{course_id}/level/{level_id}/department/{department_id}", response_model=list[StudentResultOut])
    async def get_results_for_course_level_department(
        ...
        try:
            ...
            stmt = (
                select(Enrollment).options(selectinload(Enrollment.student)) .join(Course).where(Enrollment.course_id == course_id, Course.level_id == level_id, Course.department_id == department_id))
            ...
            assignment_map = {s_id: ag.score for ag,
                              s_id in assignment_result.all()}
            ...
            for enrollment in enrollments:
                ...
                if s_id in existing_results:
                    ...
                    response.append(StudentResultOut(
                        id=result.id,
                        student_id=result.student_id,
